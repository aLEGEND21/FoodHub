...
export default function DateView({
  ...
  const [dateStats, setDateStats] = useState<DailyStats | null>(null);
  ...
  useEffect(() => {
    const fetchDateData = async () => {
      const stats = await getMealsByDate(date);
      setDateStats(stats);
      setAllMeals(stats.meals);

      // Fetch habits for the date
      const habits = await getHabitsByDate(date);
      if (habits) {
        setWorkoutDone(habits.workoutDone);
        setFruitsCount(habits.fruitsCount);
      }
    };
    fetchDateData();
  }, [date]);
  ...
  const handleWorkoutClick = async () => {
    ...
    // Save to database
    await updateHabits(date, newWorkoutDone, fruitsCount);
  };
  ...
  const handleFruitsClick = async () => {
    ...
    // Save to database
    await updateHabits(date, workoutDone, newFruitsCount);
  };
  ...
  const caloriePercent = dateStats
    ? Math.min((dateStats.totalCalories / calorieGoal) * 100, 100)
    : 0;
  const proteinPercent = dateStats
    ? Math.min((dateStats.totalProtein / proteinGoal) * 100, 100)
    : 0;
